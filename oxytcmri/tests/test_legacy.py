...
class TestSettings:
    ...
    def test_settings_errors(self, settings):
        ...
        with pytest.raises(
            AttributeError, match="No module 'invalid_module' in settings file"
        ):
            settings.invalid_module
        with pytest.raises(
            AttributeError, match="No attribute 'invalid_attribute' for module 'logs'"
        ):
    ...
    def test_to_toml(self, settings, tmp_path):
        ...
        assert (
            settings_exported.logs.LogsDirectoryPath == settings.logs.LogsDirectoryPath
        )
...
class TestLogging:
    ...
    def test_permission_error_on_directory_creation(
        self, tmp_path, test_settings_in_memory
    ):
        """
        ...
        with pytest.raises(
            PermissionError,
            match=f"Permission denied to create log directory: '{log_path}'",
        ):
    ...
    def test_permission_error_on_file_handler_creation(
        self, tmp_path, test_settings_in_memory
    ):
        """
        ...
        with pytest.raises(
            PermissionError,
            match=f"Permission denied to create log file: '{test_settings_in_memory.logs.LogsFilename}' in '{logs_directory_path}'.",
        ):
            get_logger(test_settings_in_memory)
...
class TestUtils:
    @pytest.mark.parametrize(
        "subject_type,center_id,subject_id,expected_path",
        [
            (
                "Healthy Control",
                5,
                "05_71V_MR_170615",
                "/data/Healthy/C05/05_71v_mr_170615",
            ),
            ...
            (
                ...
                23,
                "23-82T-MR-171217",
                "/data/Patient/C23/23-82t-mr-171217",
            ),
        ],
    )
    def test_get_subject_folder_path(
        self, subject_type, center_id, subject_id, expected_path
    ):
        # Setup
        data_path = "/data"
        subject = Subject(
            id=subject_id, subject_type=subject_type, center=Center(id=center_id)
        )

        # Mock the Path.exists and Path.iterdir methods to return True and a non-empty list, respectively.
        with (
            patch("pathlib.Path.exists", return_value=True),
            patch("pathlib.Path.iterdir", return_value=[1]),
        ):  # iterdir needs to return an iterable for 'any' to work
            # Act
    ...
    def test_create_tree_structure(self, tmp_path, database_session):
        ...
        create_tree_structure(
            root_directory, DatabaseController(Settings("test-data/test_settings.toml"))
        )

        # Verify the tree structure
        assert (root_directory / "Healthy" / "C01" / "01_71v_mr_170913").exists()
        assert (root_directory / "Patient" / "C02" / "02_71p_mr_040812").exists()
        assert (root_directory / "Patient" / "C03" / "03_71t_mr_280612").exists()
...
class TestModels:
    ...
    def test_create_subject(self, database_session):
        ...
        subject1 = Subject(
            id="subject_id_1", subject_type="Healthy Control", center=grenoble
        )
...
class TestDatabaseController:
    ...
    def test_create_engine_exception(self, test_settings_in_memory):
        ...
        with patch(
            "oxytcmri.controllers.create_engine", side_effect=Exception("Mocked error")
        ):
            with patch.object(test_settings_in_memory.logger, "error"):
                with pytest.raises(
                    DatabaseError,
                    match="Error while creating the database engine or tables: Mocked error",
                ):
    ...
    def test_commit_changes_sqlalchemy_error(self, db_controller_in_memory):
        ...
        with patch.object(
            db_controller_in_memory.database_session,
            "commit",
            side_effect=SQLAlchemyError("Mocked error"),
        ):
            with patch.object(
                db_controller_in_memory.database_session, "rollback"
            ) as mock_rollback:
                with patch.object(
                    db_controller_in_memory.logger, "error"
                ) as mock_logger_error:
                    with pytest.raises(SQLAlchemyError, match="Mocked error"):
                        db_controller_in_memory.commit_changes()
                    mock_rollback.assert_called_once()
                    mock_logger_error.assert_called_once_with(
                        "Error while committing changes to database: Mocked error"
                    )
    ...
    def test_add_md_lesions_volume(self, db_controller_in_memory):
        ...
        db_controller_in_memory.add_mean_diffusivity_lesions_volume(
            md_lesion_volume, overwrite_data=True
        )

        # check if the object is added
        assert db_controller_in_memory.get_all_objects(MDLesionVolume) == [
            md_lesion_volume
        ]
        ...
        db_controller_in_memory.add_mean_diffusivity_lesions_volume(
            md_lesion_volume2, overwrite_data=False
        )
        ...
        db_controller_in_memory.add_mean_diffusivity_lesions_volume(
            md_lesion_volume2, overwrite_data=True
        )
    ...
    def test_get_distinct_localizations(self, db_controller_in_memory):
        ...
        assert (
            db_controller_in_memory.get_distinct_localizations()
            == expected_localizations
        )
    ...
    def test_count_patients(self, db_controller_in_memory, settings_with_test_data):
        ...
        healthy_control = Subject(
            id="healthy_control_id", subject_type="Healthy Control", center_id=1
        )
    ...
    def test_object_add_and_exists(self, db_controller_in_memory):
        ...
        existing_subject = Subject(
            id="existing_subject", subject_type="Patient", center_id=1
        )
    ...
    def test_add_list_of_objects(self, db_controller_in_memory):
        ...
        subject1 = Subject(
            id="subject_id_1", subject_type="Healthy Control", center=test_center
        )
        subject2 = Subject(
            id="subject_id_2", subject_type="Healthy Control", center=test_center
        )
        subject3 = Subject(
            id="subject_id_3", subject_type="Healthy Control", center=test_center
        )

        db_controller_in_memory.add_objects_list([subject1, subject2, subject3])

        for id_number in [1, 2, 3]:
            assert db_controller_in_memory.object_exists(
                Subject, id=f"subject_id_{id_number}"
            )
    ...
    def test_get_mri_exam_and_volumes(self, db_controller_in_memory):
        ...
        subject = Subject(
            id=74865, subject_type="test_patient", center=babeloued_center
        )
        mri_exam = MRIExam(subject=subject)
        mri_volume = MRIVolume(name="T1", filepath="/path/to/t1.nii.gz", exam=mri_exam)
        ...
        assert (
            db_controller_in_memory.get_mri_exam(subject=subject).subject.id
            == subject.id
        )
        assert (
            db_controller_in_memory.get_mri_volume(
                subject_id=subject.id, volume_name="T1"
            ).name
            == "T1"
        )
    ...
    def test_get_all_subjects_operational_error(self, db_controller_in_memory):
        ...
        with patch.object(
            db_controller_in_memory,
            "get_all_objects",
            side_effect=OperationalError("Mocked statement", [], "Mocked error"),
        ):
            with pytest.raises(
                DatabaseError,
                match="An error occurred while fetching all subjects from the database",
            ):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with pytest.raises(Exception, match="Mocked error"):
                    db_controller_in_memory.add_object(obj)
                mock_logger_error.assert_called_once_with(
                    f"Error while adding object{obj}: Mocked error"
                )
...
class TestMRIAnalysis:
    ...
    def test_get_list_of_localizers(self, settings_with_test_data):
        ...
        brain_localizers_list_json_path = (
            settings_with_test_data.brainlocalizers.brain_localizers_list_json_path
        )
...
class TestCLI:
    ...
    def _run_command_with_exception_handling(self, command, *args, **kwargs):
        ...
        **kwargs:
        ...
        command_args = (
            [command] + list(args) + [f"--{k} {v}" for k, v in kwargs.items()]
        )
    ...
    def test_integration_import_data(
        self,
        database_session,
        settings_filepath,
        ...
        """
        self._run_command_with_exception_handling(
            "import-data",
            "--settings",
            settings_filepath,
            "--database-url",
            database_session.bind.url,
        )
    ...
    def test_integration_compute_md_lesion(
        self, tmp_path_factory, settings_filepath, local_data
    ):
        ...
        tmp_dir = tmp_path_factory.mktemp("database")
        test_settings_filepath = settings_with_copied_database(
            tmp_dir, settings_filepath=settings_filepath
        )
        ...
        number_of_md_lesion_volumes_per_subject = (
            n_lesion_types
            * n_quantiles
            * (number_of_brain_localizers_in_json + 1)  # +1 for the whole brain
        )
        expected_number_of_md_lesion_volumes = (
            number_of_patients * number_of_md_lesion_volumes_per_subject
        )
        ...
        # Run the command
        self._run_command_with_exception_handling(
            "compute-md-lesions", "--settings", test_settings_filepath
        )
    ...
    def test_integration_export_data(
        self,
        database_session,
        settings_filepath,
        csv_filepath,
        ...
        total_columns = number_of_clinical_columns + (
            number_of_brain_localizers * 2 * 2
        )
        ...
        assert (
            results_data_frame["low_MD_lesions_in_mL_7_94_whole_brain"].mean()
            == expected_mean_of_low_MD_lesions_in_mL_7_94
        )
...
class TestAddClinicalData:
    ...
    def dictionary_of_additional_clinical_data(self) -> dict:
        return {
            Subject(id="01-71-P"): 2,
            Subject(id="01-72-P"): 3,
            Subject(id="03-71-P"): 4,
        }
    ...
    def mock_clinical_data_repository(self) -> ClinicalDataRepository:
        class MockClinicalDataRepository(ClinicalDataRepository):
            def import_additional_clinical_data(
                self, clinical_data: dict, decoder
            ) -> None:
                pass
    ...
    @pytest.fixture()
    def mock_additional_clinical_data_repository(
        self, dictionary_of_additional_clinical_data
    ) -> AdditionalClinicalDataRepository:
    ...
    def test_class_additional_clinical_data(
        self, mock_clinical_data_repository, mock_additional_clinical_data_repository
    ):
        """
        ...
        additional_clinical_data = AddClinicalData(
            clinical_data_repo, additional_clinical_data_repo
        )
    ...
    def mock_csv_additional_clinical_data_repository(self):
        return CSVAdditionalClinicalDataRepository(
            filepath="test-data/clinical_data/additional_clinical_data.csv",
            subject_id_column_name="ID_SECONDAIRE",
            clinical_data_column_name="TEST_KEY",
            delimiter=";",
        )

    def test_csv_additional_clinical_data_repository(
        self, mock_csv_additional_clinical_data_repository
    ):
        """
        Test if the CSVAdditionalClinicalDataRepository class is correctly created.
        """
        expect_data = {
            Subject(id="01-71-P"): "B",
            Subject(id="01-72-P"): "A",
            Subject(id="02-71-P"): "A",
            Subject(id="03-71-P"): "B",
        }

        additional_clinical_data = (
            mock_csv_additional_clinical_data_repository.extract_data()
        )
    ...
    def test_excel_clinical_data_repository_import(
        self, additional_clinical_data_example, dictionary_of_additional_clinical_data
    ):
        """
        Test if the ExcelClinicalDataRepository class correctly imports data.
        """
        # first, copy the file to a temporary directory
        clinical_data_file_temporary_file = copy_to_temp_file(
            "test-data/clinical_data/clinical_data.xlsx"
        )
        ...
        test_key_value = data_frame.loc[
            data_frame["id_secondaire"] == "03-71-P", new_column_name
        ].values[0]
        assert (
            test_key_value
            == dictionary_of_additional_clinical_data[Subject(id="03-71-P")]
        )
    ...
    def test_clinical_data_decoder(self, additional_clinical_data_example):
        ...
        clinical_data_decoder = ClinicalDataDecoder(
            new_name="DECODED_TEST_KEY", decoder=decoder
        )
        decoded_clinical_data = clinical_data_decoder.decode(
            additional_clinical_data_example
        )

        assert decoded_clinical_data.name == "DECODED_TEST_KEY"
        assert decoded_clinical_data.get(Subject(id="01-71-P")) == 2
        assert decoded_clinical_data.get(Subject(id="01-72-P")) == 3
        assert decoded_clinical_data.get(Subject(id="03-71-P")) == 4
